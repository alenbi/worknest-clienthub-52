...
// Check if email already exists as client (fixed version)
export async function checkEmailExists(email: string): Promise<boolean> {
  ...
  try {
    ...
    // First check in clients table directly
    ...
    // Also check in auth.users via checkEmailExists RPC
    try {
...
export async function createClientWithAuth(name: string, email: string, password: string, company?: string, phone?: string, domain?: string) {
  try {
    ...
    console.log("Email check passed, proceeding with client record creation first");
    
    // First create the client record without user_id
    const { data: clientData, error: clientError } = await supabase
      .from('clients')
      .insert({
        name,
    ...
    console.log("Client record created successfully with ID:", clientData.id);
    
    // Now create auth user
    console.log("Creating auth user for email:", standardEmail);
    ...
    // Link client to user_id
    console.log("Linking client", clientData.id, "to user", userId);
    ...
    if (updateError) {
      ...
      // We don't throw here as the client record and auth user are created
    } else {
      console.log("Client linked to user successfully");
    }
